...
The day becomes a trajectory. This is descriptive only and uses canonical C1/C2/C3 views.

## Canonical Inputs

- `derived.v_ovc_c1_features_v0_1`
- `derived.v_ovc_c2_features_v0_1`
- `derived.v_ovc_c3_features_v0_1` (labels only)
...
- Range intensity: `rng_rank_6` (C2)
- Body participation: `body_ratio` (C1)
- Directional efficiency: `abs(directional_efficiency)` (C1)
...
Weighted blend of C3 labels mapped to numeric values:

- `c3_trend_bias`: sustained -> -1.0, nascent -> -0.5, neutral -> 0.0, fading -> 0.5
- `c3_momentum_state`: accelerating -> -1.0, steady -> 0.0, decelerating -> 0.5, reversing -> 1.0
...
- C3 label noise can move `y_shift` across the `S_hi` boundary.
