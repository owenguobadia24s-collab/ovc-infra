# C3 Entry Checklist (v0.1)

> **Purpose**: This checklist MUST be satisfied before any new C3 tag is merged to main.  
...
Before opening a PR for a new C3 tag, verify ALL items are complete:

### 1. Design & Contract Compliance

- [ ] **C3 Semantic Contract reviewed**: Read `docs/c3_semantic_contract_v0_1.md` in full
- [ ] **Tag is semantic**: Classification represents meaningful market state (not a raw metric)
- [ ] **Inputs are C1/C2 only**: No direct B-layer OHLC access in classification logic
...
- [ ] **Config file exists**: `configs/threshold_packs/c3_<tag_name>_v1.json`
...
- [ ] **Pack ID matches tag**: e.g., `c3_regime_trend` pack for `c3_regime_trend` tag

### 3. SQL Migration

- [ ] **Migration file exists**: `sql/XX_c3_<tag_name>_v0_1.sql`
- [ ] **Table in derived schema**: `derived.ovc_c3_<tag_name>_v0_1`
...
- [ ] **Classification constraint**: `CHECK (c3_<tag> IN ('VALUE1', 'VALUE2', ...))`
...
- [ ] **Script exists**: `src/derived/compute_c3_<tag_name>_v0_1.py`
- [ ] **Reference implementation followed**: Pattern matches `compute_c3_regime_trend_v0_1.py`
...
- [ ] **C1/C2 data fetch only**: No B-layer OHLC queries in classification logic
- [ ] **Upsert semantics**: `ON CONFLICT (symbol, ts) DO UPDATE`
- [ ] **CLI arguments present**: `--symbol`, `--threshold-pack`, `--scope`, `--dry-run`
- [ ] **Script imports cleanly**: `python -c "from src.derived.compute_c3_<tag>_v0_1 import *"`
- [ ] **--help works**: `python src/derived/compute_c3_<tag>_v0_1.py --help`

### 5. Tests

- [ ] **Test file exists**: `tests/test_c3_<tag_name>.py`
...
- [ ] **All tests pass**: `python -m pytest tests/test_c3_<tag_name>.py -v`
...
- [ ] **Validation runs without error**: `--validate-c3 --c3-classifiers c3_<tag_name>`
...
```
feat(c3): add c3_<tag_name> classifier

- Add SQL migration for derived.ovc_c3_<tag_name>_v0_1
- Add compute script with threshold registry integration
- Add threshold pack config
- Add tests (N passing)
- Extend validator for C3 coverage
```
...
python -m src.config.threshold_registry_cli create `
  --pack-id c3_<tag_name> `
  --version 1 `
  --scope GLOBAL `
  --config-file configs/threshold_packs/c3_<tag_name>_v1.json
...
# 4. Run compute
python src/derived/compute_c3_<tag_name>_v0_1.py `
  --symbol GBPUSD `
  --threshold-pack c3_<tag_name> `
  --scope GLOBAL

# 5. Run tests
python -m pytest tests/test_c3_<tag_name>.py -v
...
C3 tags are **semantic infrastructure**. A broken C3 tag can:
...
This checklist ensures every C3 tag meets the same quality bar as `c3_regime_trend`, the reference implementation.
