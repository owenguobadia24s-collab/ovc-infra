...
- **`ovc_cfg.threshold_pack`** — Static config for C3 regime classification
...
- **`derived.ovc_c1_features_v0_1`** (table) — Python-computed, validated by QA scripts
- **`derived.ovc_c2_features_v0_1`** (table) — Python-computed, validated by QA scripts
- **`derived.v_ovc_c1_features_v0_1`** (view) — SQL definition, recomputes from canonical
- **`derived.v_ovc_c2_features_v0_1`** (view) — SQL definition, recomputes from canonical
- **`derived.v_ovc_c3_features_v0_1`** (view) — SQL definition, uses threshold registry
...
- **`state_tag`** in `ovc.ovc_blocks_v01_1_min` — Conceptually C2/C3 derived; stored in canonical table
- **`trend_tag`** in `ovc.ovc_blocks_v01_1_min` — Conceptually C2/C3 derived; stored in canonical table
- **`pred_dir`** in `ovc.ovc_blocks_v01_1_min` — Conceptually C3 derived; stored in canonical table
- **`bias_dir`** in `ovc.ovc_blocks_v01_1_min` — Conceptually C3 derived; stored in canonical table
- **`struct_state`** in `ovc.ovc_blocks_v01_1_min` — Conceptually C2 derived; stored in canonical table
- **Legacy vs split derived** — Both `derived.ovc_block_features_v0_1` AND split C1/C2/C3 exist
- **C3 regime classification** — Compute script unused; only SQL view available
