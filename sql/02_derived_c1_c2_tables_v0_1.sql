-- OVC Option B.1: Derived C1/C2 Feature Pack Tables (v0.1)
-- Generated: 2026-01-18
-- Purpose: Create tables for C1 (single-bar) and C2 (multi-bar) derived features
-- 
...
CREATE TABLE IF NOT EXISTS derived.derived_runs_v0_1 (
    ...
    run_type            TEXT NOT NULL,              -- 'c1', 'c2', 'c3', etc.
    version             TEXT NOT NULL,              -- 'v0.1'
    formula_hash        TEXT NOT NULL,              -- MD5 hash of computation logic
    window_spec         TEXT,                       -- NULL for C1, required for C2+
    threshold_version   TEXT,                       -- NULL for C1/C2, required for C3
...
--------------------------------------------------------------------------------
-- OVC_C1_FEATURES_V0_1: Single-bar OHLC primitives
--------------------------------------------------------------------------------
-- C1 KEEP set from metric_trial_log_noncanonical_v0.md Section E.1:
...
CREATE TABLE IF NOT EXISTS derived.ovc_c1_features_v0_1 (
    ...
    formula_hash        TEXT NOT NULL,              -- MD5 of C1 formula set
    derived_version     TEXT NOT NULL DEFAULT 'v0.1',
    
    -- C1 Features: Single-bar OHLC primitives
...
CREATE INDEX IF NOT EXISTS idx_c1_features_v0_1_run_id 
    ON derived.ovc_c1_features_v0_1(run_id);

--------------------------------------------------------------------------------
-- OVC_C2_FEATURES_V0_1: Multi-bar structure & context
--------------------------------------------------------------------------------
-- C2 KEEP set from mapping_validation_report_v0.1.md Section 1.1 and 1.2:
...
CREATE TABLE IF NOT EXISTS derived.ovc_c2_features_v0_1 (
    -- Identity (FK to B-layer)
    block_id            TEXT PRIMARY KEY,
    ...
    -- C2 Features: Rolling 12-bar stats (window_spec: N=12)
    ...
    -- C2 Features: 12-bar structure breaks (window_spec: N=12)
    hh_12               BOOLEAN,                    -- h > max(h[-12:-1])
    ll_12               BOOLEAN,                    -- l < min(l[-12:-1])
    
    -- C2 Features: Range detector numeric (window_spec: parameterized=rd_len)
...
CREATE INDEX IF NOT EXISTS idx_c2_features_v0_1_run_id 
    ON derived.ovc_c2_features_v0_1(run_id);

--------------------------------------------------------------------------------
-- COMMENTS: Document window_spec per feature family
--------------------------------------------------------------------------------
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.gap IS 'window_spec: N=1 (prev_c)';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.took_prev_high IS 'window_spec: N=1';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.took_prev_low IS 'window_spec: N=1';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.sess_high IS 'window_spec: session=date_ny';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.sess_low IS 'window_spec: session=date_ny';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.dist_sess_high IS 'window_spec: session=date_ny (inherits)';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.dist_sess_low IS 'window_spec: session=date_ny (inherits)';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.roll_avg_range_12 IS 'window_spec: N=12';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.roll_std_logret_12 IS 'window_spec: N=12';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.range_z_12 IS 'window_spec: N=12 (inherits)';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.hh_12 IS 'window_spec: N=12';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.ll_12 IS 'window_spec: N=12';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.rd_hi IS 'window_spec: parameterized=rd_len';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.rd_lo IS 'window_spec: parameterized=rd_len';
COMMENT ON COLUMN derived.ovc_c2_features_v0_1.rd_mid IS 'window_spec: parameterized=rd_len (derived)';
